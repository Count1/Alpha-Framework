...
RegisterNetEvent("Alpha:SendAllPickups")
AddEventHandler("Alpha:SendAllPickups", function(pick, id, del, newCount)
...
function LoadPickups()
    function GetItemProp(item)
        for k,v in pairs(items_sys.items_config) do
            if k == item then
                if v.prop ~= nil then
                    return v.prop
                else
                ...
                if #(v.coords - pCoords) < 2 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if amount <= v.count then
                            TriggerServerEvent("Alpha:TakePickup", v.id, v.item, amount, v.count)
                            if amount == v.count then
                                if v.id == nil then break end
