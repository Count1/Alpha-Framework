...
local items = items_sys.items_config
...
function AddItem(id, item, count, args)
    if items[item] ~= nil then
        ...
        TriggerClientEvent("Alpha:OnGetItem", id, items[item].label, count)
        TriggerClientEvent("Alpha:OnInvRefresh", id, PlayersCache[id].inv, GetInvWeight(PlayersCache[id].inv))
    else
...
function RemoveItem(id, item, itemid, count)
    if items[item] ~= nil then
        if PlayersCache[id].inv[itemid] ~= nil then -- Item do not exist in inventory
            ...
            TriggerClientEvent("Alpha:OnRemoveItem", id, items[item].label, count)
            TriggerClientEvent("Alpha:OnInvRefresh", id, PlayersCache[id].inv, GetInvWeight(PlayersCache[id].inv))
            return true
        else
            ErrorHandling(id, 2)
...
function AddItemIf(id, item, count, args)
    if items[item] ~= nil then
        ...
        if iWeight + (items[item].weight * count) <= items_sys.defaultWeightLimit then
            if not exist then -- Item do not exist in inventory, creating it
                itemid = GenerateItemId()
                PlayersCache[id].inv[itemid] = {}
                PlayersCache[id].inv[itemid].item = item
            ...
            TriggerClientEvent("Alpha:OnWeightLimit", id, items[item].label)
            return false
        end
    else
        -- Item do not exist, should do some kind of error notification
...
function ExhangeItem(id, target, item, count, countsee, args, itemid)
    if items[item] ~= nil then
        ...
        if tWeight + (items[item].weight * count) <= items_sys.defaultWeightLimit then
            ...
            TriggerClientEvent("Alpha:OnGetItem", target, items[item].label, count)
            TriggerClientEvent("Alpha:OnInvRefresh", target, PlayersCache[id].inv, tWeight)
        else
