...
function BankToCash(id,value)
    if value > PlayersCache[id].bank then
        ...
        RemoveBank(id,value)
        AddMoney(id,value)
        TriggerClientEvent("Alpha:OnAccountsRefresh", id, PlayersCache[id].money, PlayersCache[id].bank)
    end
end
...
function BankTransfer(from,to,value)
    if value > PlayersCache[from].bank then
        ...
        AddBank(to,value)
        TriggerClientEvent("Alpha:OnAccountsRefresh", from, PlayersCache[from].money, PlayersCache[from].bank)
        TriggerClientEvent("Alpha:OnAccountsRefresh", to, PlayersCache[to].money, PlayersCache[to].bank)
    end
end
...
function ChangePlayerJob(id, job, grade)
    ...
    TriggerClientEvent("Alpha:OnJobChange", id, job, grade)
end
...
RegisterNetEvent("Alpha:SavePos")
AddEventHandler("Alpha:SavePos", function(pos)
